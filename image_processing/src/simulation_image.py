...
#the image Width and Height are fixed in the callbacks, if the scale change in the webots controller_python, the value in gray_callback have to be changed.
#here we consider the focallenght to be ideal AND for this width and heigth, change it in detect_circle 
#the H field of view is set in detect_circle/ellipse as well (wanted to avoid global) in degree.
...
def detect_circles(imgThresholded, width):
    ...
    simulation_HFOV=45
    focallenght = 75#perfect focallenght, for the fictive frame 180x120
