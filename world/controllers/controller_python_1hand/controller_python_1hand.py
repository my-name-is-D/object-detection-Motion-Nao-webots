...
class Nao (Robot,Motion,PositionSensor):
    ...
    def jointcallback(self,data):
        ...
        if not ( data.data =="POSITION NOT REACHABLE"):
            if self.Left_Resetallowed==True and self.number_reset<2: #If the previous motion has been fully executed 
 ...
 ########################## FUNCTION CLOSE OPEN HAND ###################    
    ...
    def move_with_callback(self):
        ...
        if (self.prev_Larmangles!=self.Larmangles):
            ...
            for i in xrange(0,len(self.Larmangles)-1,4):
                #Open hand after beginning of motion
                if i>=len(self.Larmangles)/4:#half motion
 ...
 #-----------------------------LEFT ARM-----------------------------------------
            ...
            #only 1 motion autohrized
            if len(self.Larmangles)>5:
                print("Lift")
                self.LShoulderPitch.setPosition(0.29)
                self.I_went_there=1
                
                
            print("there")    
            if self.I_went_there!=1:
